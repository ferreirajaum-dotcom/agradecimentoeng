    ...
    <main className="min-h-screen text-brand-muted selection:bg-brand-gold selection:text-brand-dark font-sans overflow-x-hidden bg-brand-dark flex flex-col">
      ...
      <div className="flex-grow flex flex-col items-center justify-center pt-28 pb-20 px-4 md:px-6 relative">
        ...
        <div className="w-full max-w-3xl mx-auto space-y-12 z-10 relative">
          ...
          {/* Minimalist Author Quote */}
          <motion.div 
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            transition={{ delay: 0.8 }}
            className="pt-8 text-center border-t border-white/5 mt-12"
          >
             <div className="flex flex-col items-center gap-6">
                <div className="w-14 h-14 rounded-full overflow-hidden border border-white/10 grayscale opacity-50 hover:opacity-80 transition-opacity duration-500">
                  <img src={AUTHOR.image} alt={AUTHOR.name} className="w-full h-full object-cover" />
                </div>
                <div className="space-y-3 max-w-2xl mx-auto">
                   <p className="text-lg md:text-xl font-serif italic text-brand-muted/70 leading-relaxed">
                      "Não é sobre a velocidade com que você sobe a montanha, mas sobre a clareza de saber por que está subindo."
                   </p>
                   <div className="flex items-center justify-center gap-2 text-[10px] font-mono uppercase tracking-widest text-brand-gold/40">
